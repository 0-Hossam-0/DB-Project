...
SELECT 
    ...
    MAX(p.post_id) AS max_post_id,
    AVG(p.post_id) AS avg_post_id
FROM Users u
JOIN User_Post p ON u.user_id = p.user_id
GROUP BY u.user_id
HAVING COUNT(p.post_id) >= 1  
ORDER BY u.user_id;

-- Number of followers per company --
